...
function buildAnalysisPayload(model, qLive=3.0, supportMode='PINNED', connCfg=null, extraLoads=null){
  ...
  // With purely PIN beam connections, many 3D layouts become mechanisms.
  // When any bracing is present, default beams/sub-beams to FIXED to provide a stable
  // moment frame backbone unless the user explicitly overrides connections.
  try{
    const brs = Array.isArray(m?.braces) ? m.braces : [];
    if(brs.length){
      defaultModeByKind.beamX = 'FIXED';
      defaultModeByKind.beamY = 'FIXED';
      defaultModeByKind.subBeam = 'FIXED';
    }
  }catch{}
  ...
  try{
    for(const pl of pipeLoads){
      ...
      for(const mm of memList){
        ...
        casePIPE.memberUDL.push({ memberId: String(mm.id), dir, w });
      }
    }
  ...
  if(casePIPE.nodeLoads.length || casePIPE.memberUDL.length) cases.push(casePIPE);
...
function renderAnalysis(projectId){
  ...
  setTopbarActions(`
    ...
    <button class="pill" id="btnReportKo" type="button">PDF 계산서(KO)</button>
    <button class="pill" id="btnReportEn" type="button">PDF Report(EN)</button>
  ...
  (async () => {
      ...
      try{
        ...
        if(!res || res.ok !== true){
          ...
          if(String(note).toLowerCase().includes('singular')){
            showRunHelp(`
              <div class="card" style="margin-top:10px; padding:10px; border-color: rgba(239,68,68,0.25)">
                ...
                <div class="row" style="margin-top:8px; gap:8px; flex-wrap:wrap">
                  ...
                  <button class="btn" id="btnDlPayload" type="button">Download payload.json</button>
                </div>
                <div class="note" style="margin-top:8px">Retries run temporarily (won't overwrite your saved settings). Payload helps debugging.</div>
              </div>
            `);
            ...
            document.getElementById('btnDlPayload')?.addEventListener('click', () => {
              try{ download(`prebim-analysis-payload-${Date.now()}.json`, JSON.stringify(payloadSend, null, 2)); }
    ...
    const buildReportHtml = (res, payload, lang='ko') => {
      ...
      const build = (typeof BUILD==='string' ? BUILD : '');
...
<title>${escapeHtml(projName)} - ${t('계산서','Report')}</title>
<style>
  ...
  th,td{ border:1px solid rgba(148,163,184,0.45); padding:4px 6px; font-size:10px; }
  ...
  .k{ width:160px; color: rgba(11,27,58,0.7); }
  .v{ flex:1; }
  .member-table th, .member-table td{ font-size: 8px; padding: 3px 4px; }
</style>
</head>
<body>
  <div class="title">${t('PreBIM 구조계산서','PreBIM Structural Calculation Report')}</div>
  <div class="banner">
    <div class="row">
      <div class="small">${t('PreBIM에서 자동 생성된 보고서','Auto-generated by PreBIM')}</div>
      <div class="small muted">BUILD: <b class="mono">${escapeHtml(build)}</b></div>
    </div>
    <div class="meta-grid small">
      <div class="kv"><div class="k">${t('프로젝트','Project')}</div><div class="v"><b>${escapeHtml(projName)}</b></div></div>
      <div class="kv"><div class="k">${t('내보낸 시각','Exported')}</div><div class="v mono">${escapeHtml(nowIso)}</div></div>
      <div class="kv"><div class="k">${t('해석 조합','Analysis combo')}</div><div class="v mono"><b>${escapeHtml(combo)}</b></div></div>
      <div class="kv"><div class="k">${t('기반 엔진','Engine')}</div><div class="v">Model: <span class="mono">PreBIM engine.js</span> · Solver: <span class="mono">PyNite FEModel3D</span> (<span class="mono">prebim-analysis-api</span>)</div></div>
    </div>
  </div>

  <div class="card">
    <h2 class="small">${t('입력','Inputs')}</h2>
    <table style="margin-top:6px">
      <tbody>
        <tr>
          <th style="width:180px">${t('설계방법','Design method')}</th><td class="mono">${escapeHtml(dm)}</td>
          <th style="width:180px">${t('지점','Supports')}</th><td class="mono">${escapeHtml(supportMode)}</td>
          <th style="width:180px">${t('활하중 qL (kN/m²)','Live load qL (kN/m²)')}</th><td class="mono">${escapeHtml(String(qLive))}</td>
          <th style="width:180px">${t('적설하중 qS (kN/m²)','Snow load qS (kN/m²)')}</th><td class="mono">${escapeHtml(String(qSnow))}</td>
        </tr>
        <tr>
          <th>${t('풍하중 X (kN)','Wind X (kN)')}</th><td class="mono">${escapeHtml(String(windX))}</td>
          <th>${t('풍하중 Z (kN)','Wind Z (kN)')}</th><td class="mono">${escapeHtml(String(windZ))}</td>
          <th>${t('지진하중 X (kN)','Seismic X (kN)')}</th><td class="mono">${escapeHtml(String(eqX))}</td>
          <th>${t('지진하중 Z (kN)','Seismic Z (kN)')}</th><td class="mono">${escapeHtml(String(eqZ))}</td>
        </tr>
      </tbody>
    </table>
  </div>
  ...
  <div class="card">
    <h2 class="small">${t('해석 요약','Summary')}</h2>
    <div class="row" style="margin-top:6px">
      <div class="small">${t('최대 변위','Max displacement')}: <b class="mono">${fmt(maxDisp,6)} m</b> @ node <b class="mono">${escapeHtml(maxNode)}</b></div>
      <div class="small muted">${t('부재 수','Members')}: <b class="mono">${members.length}</b> · ${t('노드 수','Nodes')}: <b class="mono">${(payload?.nodes||[]).length}</b></div>
    </div>
  </div>

  <div class="card">
    <h2 class="small">${t('하중조합','Load combinations')}</h2>
    <table style="margin-top:6px">
  ...
  <div class="card pagebreak">
    <h2 class="small">${t('부재별 결과(전체)','Member results (all members)')}</h2>
    <div class="muted small" style="margin-top:4px">${t('End force는 solver local member force입니다. MaxAbs는 부재 길이방향 샘플링 최대값입니다.','End forces are local member forces. MaxAbs are sampled maxima along the member.')}</div>
  ...
  <div class="printnote muted">${t('PDF로 저장하려면 브라우저 인쇄(Chrome: Ctrl+P)에서 “대상: PDF 저장”을 선택하세요.','Use Print → Save as PDF (Chrome: Ctrl+P).')}</div>
</body>
</html>`;
    };

    const exportReport = (lang='ko') => {
      if(!window.__lastAnalysisRes || !lastPayload){
        alert(lang==='ko' ? '먼저 Run을 실행해서 해석 결과를 만든 뒤 계산서를 내보낼 수 있어요.' : 'Run analysis first, then export the report.');
        return;
      }
      try{
        const html = buildReportHtml(window.__lastAnalysisRes, lastPayload, lang);
        const w = window.open('', '_blank');
        if(!w){ alert(lang==='ko' ? '팝업이 차단되었습니다. 팝업 허용 후 다시 시도해주세요.' : 'Popup blocked. Please allow popups and retry.'); return; }
        w.document.open();
        ...
        console.error(e);
        alert(lang==='ko' ? '계산서 생성에 실패했습니다.' : 'Failed to generate report.');
      }
    };

    document.getElementById('btnReportKo')?.addEventListener('click', () => exportReport('ko'));
    document.getElementById('btnReportEn')?.addEventListener('click', () => exportReport('en'));
  ...
  (async () => {
    ...
    const rebuildBoxMemSection = () => {
      ...
      if(sh && sh.options.length===0){
        ...
        if(keys.includes('H')) sh.value='H';
      }
      const items = data[stdKey]?.shapes?.[sh?.value||'H']?.items || [];
      if(sz){
        const prev=sz.value;
        sz.innerHTML='';
        items.forEach(it=>{ const o=document.createElement('option'); o.value=it.key; o.textContent = `${it.name}${(it.kgm!=null && Number.isFinite(it.kgm)) ? ` · ${it.kgm} kg/m` : ''}`; sz.appendChild(o); });
...
async function createThreeView(container){
  ...
  function showBoxHotLine(a, b, color=0xff3b30, r=0.05){
    ...
    // Use a thin cylinder so width is visible (LineBasicMaterial linewidth is ignored on most platforms)
    const A = new THREE.Vector3(...a);
    const B = new THREE.Vector3(...b);
    const dir = B.clone().sub(A);
    const len = dir.length();
    ...
    const mesh = new THREE.Mesh(g, m);
    mesh.position.copy(mid);
    // cylinder default axis is Y; rotate Y to align with dir
  ...
  function showBoxHotPreviewBox(b){
    ...
    g.translate((b.x0+b.x1)/2, (b.y0+b.y1)/2, (b.z0+b.z1)/2);
    const edges = new THREE.EdgesGeometry(g, 1);
    const mat = new THREE.LineBasicMaterial({ color:0x7c3aed, transparent:true, opacity:0.85, depthTest:false });
